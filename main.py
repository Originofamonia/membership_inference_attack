...
path = config.path.result_path + str(config.statistics.dataset) + "_" + str(
    config.statistics.type) + "_" + str(now)
os.mkdir(path)
...
if config.statistics.dataset == "MNIST":
    if config.statistics.type == "training_size":
        print("START STATS ON TRAINING SIZE WITH MNIST : ",
              config.statistics.training_size_value)
        ...
        res_precision_per_class = np.zeros(
            (len(config.statistics.training_size_value), 10))
        res_recall_per_class = np.zeros(
            (len(config.statistics.training_size_value), 10))
        res_accuracy_per_class = np.zeros(
            (len(config.statistics.training_size_value), 10))
        for index_value_to_test, value_to_test in enumerate(
                config.statistics.training_size_value):
            config.set_subkey("general", "train_target_size", value_to_test)
            precision_general, recall_general, accuracy_general, precision_per_class, recall_per_class, accuracy_per_class = experience_mnist(
                config, path, index_value_to_test)
            ...
            np.save(path + "/res_precision_per_class.npy",
                    res_precision_per_class)
            np.save(path + "/res_accuracy_per_class.npy",
    ...
    if config.statistics.type == "number_shadow":
        ...
        res_precision_per_class = np.zeros(
            (len(config.statistics.number_shadow_value), 10))
        res_recall_per_class = np.zeros(
            (len(config.statistics.number_shadow_value), 10))
        res_accuracy_per_class = np.zeros(
            (len(config.statistics.number_shadow_value), 10))
    ...
    if config.statistics.type == "overfitting":
        print("START STATS ON OVERFITTING WITH MNIST : ",
              config.statistics.epoch_value)
        ...
        res_precision_per_class = np.zeros(
            (len(config.statistics.epoch_value), 10))
        res_recall_per_class = np.zeros(
            (len(config.statistics.epoch_value), 10))
        res_accuracy_per_class = np.zeros(
            (len(config.statistics.epoch_value), 10))
...
if config.statistics.dataset == "CIFAR10":
    if config.statistics.type == "training_size":
        print("START STATS ON TRAINING SIZE WITH CIFAR10 : ",
              config.statistics.training_size_value)
        res_precision = np.zeros(len(config.statistics.training_size_value))
        res_recall = np.zeros(len(config.statistics.training_size_value))
        res_accuracy = np.zeros(len(config.statistics.training_size_value))
        ...
        for index_value_to_test, value_to_test in enumerate(
                config.statistics.training_size_value):
            config.set_subkey("general", "train_target_size", value_to_test)
            precision_general, recall_general, accuracy_general, precision_per_class, recall_per_class, accuracy_per_class = experience_cifar10(
                config, path, index_value_to_test)
            res_precision[index_value_to_test] = precision_general
            res_recall[index_value_to_test] = recall_general
            res_accuracy[index_value_to_test] = accuracy_general
            ...
            # np.save(path + "/res_best_acc_shadows.npy", res_best_acc_shadows)
    if config.statistics.type == "number_shadow":
        print("START STATS ON NUMBER SHADOW  WITH CIFAR10 : ",
              config.statistics.number_shadow_value)
        res_precision = np.zeros(len(config.statistics.number_shadow_value))
        res_recall = np.zeros(len(config.statistics.number_shadow_value))
        res_accuracy = np.zeros(len(config.statistics.number_shadow_value))
        ...
        for index_value_to_test, value_to_test in enumerate(
            ...
            # np.save(path + "/res_best_acc_shadows.npy", res_best_acc_shadows)
    if config.statistics.type == "overfitting":
        print("START STATS ON OVERFITTING WITH CIFAR10 : ",
              config.statistics.epoch_value)
        res_precision = np.zeros(len(config.statistics.epoch_value))
        res_recall = np.zeros(len(config.statistics.epoch_value))
        res_accuracy = np.zeros(len(config.statistics.epoch_value))
...
if config.statistics.dataset == "CIFAR100":
    if config.statistics.type == "training_size":
        print("START STATS ON TRAINING SIZE WITH CIFAR100 : ",
              config.statistics.training_size_value)
        res_precision = np.zeros(len(config.statistics.training_size_value))
        res_recall = np.zeros(len(config.statistics.training_size_value))
        res_accuracy = np.zeros(len(config.statistics.training_size_value))
        ...
        # res_best_acc_shadows = np.zeros(len(config.statistics.training_size_value))
        res_precision_per_class = np.zeros(
            (len(config.statistics.training_size_value), 100))
        res_recall_per_class = np.zeros(
            (len(config.statistics.training_size_value), 100))
        res_accuracy_per_class = np.zeros(
            (len(config.statistics.training_size_value), 100))
        for index_value_to_test, value_to_test in enumerate(
                config.statistics.training_size_value):
            config.set_subkey("general", "train_target_size", value_to_test)
            precision_general, recall_general, accuracy_general, precision_per_class, recall_per_class, accuracy_per_class = experience_cifar100(
                config, path, index_value_to_test)
            res_precision[index_value_to_test] = precision_general
            res_recall[index_value_to_test] = recall_general
            res_accuracy[index_value_to_test] = accuracy_general
            ...
            # np.save(path + "/res_best_acc_shadows.npy", res_best_acc_shadows)
    if config.statistics.type == "number_shadow":
        print("START STATS ON NUMBER SHADOW  WITH CIFAR100 : ",
              config.statistics.number_shadow_value)
        res_precision = np.zeros(len(config.statistics.number_shadow_value))
        res_recall = np.zeros(len(config.statistics.number_shadow_value))
        res_accuracy = np.zeros(len(config.statistics.number_shadow_value))
        ...
        # res_best_acc_shadows = np.zeros(len(config.statistics.number_shadow_value))
        res_precision_per_class = np.zeros(
            (len(config.statistics.number_shadow_value), 100))
        res_recall_per_class = np.zeros(
            (len(config.statistics.number_shadow_value), 100))
        res_accuracy_per_class = np.zeros(
            (len(config.statistics.number_shadow_value), 100))
        for index_value_to_test, value_to_test in enumerate(
            ...
            # np.save(path + "/res_best_acc_shadows.npy", res_best_acc_shadows)
    if config.statistics.type == "overfitting":
        print("START STATS ON OVERFITTING WITH CIFAR100 : ",
              config.statistics.epoch_value)
        res_precision = np.zeros(len(config.statistics.epoch_value))
        res_recall = np.zeros(len(config.statistics.epoch_value))
        res_accuracy = np.zeros(len(config.statistics.epoch_value))
        ...
        # res_best_acc_shadows = np.zeros(len(config.statistics.epoch_value))
        res_precision_per_class = np.zeros(
            (len(config.statistics.epoch_value), 100))
        res_recall_per_class = np.zeros(
            (len(config.statistics.epoch_value), 100))
        res_accuracy_per_class = np.zeros(
            (len(config.statistics.epoch_value), 100))
